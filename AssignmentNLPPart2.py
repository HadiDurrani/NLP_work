...
class UnigramModel(LanguageModel):
    ...
    def generateSentence(self):
        sentence = [self.unigram_dist.draw() for _ in range(random.randint(5, 15))]
        return sentence
    ...
    def getCorpusPerplexity(self, corpus):
        ...
        total_words = 0
        for sen in corpus:
            total_words += len(sen) - 2  # Exclude start and end tokens
            for word in sen:
                word_prob = self.unigram_dist.prob(word)
                if word_prob > 0:
                    total_log_prob += -1 * math.log(word_prob)
                else:
                    # Handle the case when the probability is not valid (e.g., 0)
                    # You can choose to skip the word or handle it in a way that suits your application
                    pass
...
class SmoothedUnigramModel(LanguageModel):
    ...
    def getCorpusPerplexity(self, corpus):
        ...
        for sen in corpus:
            total_words += len(sen) - 2  # Exclude start and end tokens
            for word in sen:
                total_log_prob += -1 * math.log((self.unigram_dist.counts[word] + 1) / (self.unigram_dist.total + self.vocab_size))
        perplexity = math.exp(total_log_prob / total_words)
        return perplexity
    #endddef
#endclass
...
class BigramModel(LanguageModel):
    ...
    def getSentenceProbability(self, sen):
        ...
        for i in range(1, len(sen)):
            probability *= math.exp(self.bigram_dist.prob(sen[i], sen[i-1]))
        return probability
    ...
    def getCorpusPerplexity(self, corpus):
        ...
        for sen in corpus:
            total_words += len(sen) - 2  # Exclude start and end tokens
            for i in range(1, len(sen)):
                total_log_prob += self.bigram_dist.prob(sen[i], sen[i-1])
        perplexity = math.exp(-total_log_prob / total_words)
        return perplexity
    # enddef
# endclass
    ...
    def getCorpusPerplexity(self, corpus):
        ...
        for sen in corpus:
            ...
            for i in range(1, len(sen)):
                ...
                total_log_prob += math.log(interpolated_prob) if interpolated_prob > 0 else 0.0
...
if __name__ == "__main__":
    ...
    print("Sentence Probability:", unigram_model.getSentenceProbability(["mary", "had", "a", "little", "lamb"]))
    ...
    print("Sentence Probability:", smoothed_unigram_model.getSentenceProbability(["mary", "had", "a", "little", "lamb"]))
    ...
    print("Sentence Probability:", bigram_model.getSentenceProbability(["mary", "had", "a", "little", "lamb"]))
    ...
    print("Sentence Probability:", smoothed_bigram_model.getSentenceProbability(["mary", "had", "a", "little", "lamb"]))
    ...
    #Question Answers
    #Q1) the sentence length is predetermined to be with in 5 15 randomly whereas in bigram sentence length is based on probability of next word if the current word is </s> the sentce terminates
    #Q2) Yes the probability differs as unigram gives independant probabilty to each word and bigram considers the context of the previous word and in case of smoothing(Laplace and linear interpolation) handling of unseen events improves probabilty
    #Q3) Bigram and smoothed bigram are producing more realistic sentences
    #Q4) For unigram Training Corpus has the highest perplexity for smoothed unigram Training has the highest for bigram Training has the highest for smoothed bigram negative corpus has the highest perplexity